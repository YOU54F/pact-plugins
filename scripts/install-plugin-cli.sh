...
case "$(uname -s)" in
   ...
   Linux)
     ...
     if [ "$(uname -m)" = "aarch64" ]; then
      curl -L -o ~/bin/pact-plugin-cli.gz https://github.com/pact-foundation/pact-plugins/releases/download/pact-plugin-cli-v${VERSION}/pact-plugin-cli-linux-aarch64.gz
     else
   ...
   CYGWIN*|MINGW32*|MSYS*|MINGW*)
     ...
     if [ "$(uname -m)" = "aarch64" ]; then
      curl -L -o ~/bin/pact-plugin-cli.exe.gz https://github.com/pact-foundation/pact-plugins/releases/download/pact-plugin-cli-v${VERSION}/pact-plugin-cli-windows-aarch64.exe.gz
     else
